 ...
 * @LastEditors: sueRimn
 * @LastEditTime: 2022-02-26 08:42:50
 */
#ifndef UTILS_H
#define UTILS_H
 	...
 	bool is_directory(char* path){
        ...
        */
		int load_query_pair_time(char* query_freq_filename,char* query_pair_filename,std::vector<std::pair<int, int> >& queries,int hf_rate,int &numOfHFpoint,int multi=1){
			...
			std::ifstream in_freq(query_freq_filename);//input HFPoint file to std::ifstream
			if(!in_freq.is_open()) {std::cerr<<"Cannot open "<<query_freq_filename<<std::endl;}
			//read each line representing HFpoint to std::vector 
			...
			std::ifstream in_pair(query_pair_filename);//input query file to std::ifstream
...
//******************parallel functions****************
    ...
    //compare std::string by ascending number value order
    struct CmpByKeyNum 
    {  
        bool operator()(const std::string& k1, const std::string& k2) 
        {  
  ...
  */
	void get_order_from_file(char* load_filename,std::vector<int>& rank,std::vector<int>& inv,int n){
			...
			std::ifstream in(load_filename);//input betwenness file to std::ifstream
        ...
        */
		int load_query_pair_time_overlay(char* load_filename,char* newToOriginalFileName,std::vector<std::pair<int, int> >& queries,int multi=1){
			...
			in.close();
			int cnt=0;//count the total times
			std::ifstream in_query(load_filename);//input query file to std::ifstream
        ...
        */
         ...
         *@date: 2021-01-07
        */
		int load_query_pair_time(char* load_filename,std::vector<std::pair<int, int> >& queries,int multi){
			int cnt=0;//count the total times
			std::ifstream in(load_filename);//input query file to std::ifstream
			if(!in.is_open()) {std::cerr<<"Cannot open "<<load_filename<<std::endl;}
			int s,t; 
			int query_time=0; 
			char line[24];
		...
		*/
		void load_hfpoint(char* load_filename,int& numOfHfpoint){//5%%
			...
			std::ifstream in(load_filename);//input HFPoint file to std::ifstream
			if(!in.is_open()) {std::cerr<<"Cannot open "<<load_filename<<std::endl;}
			int id;int t,i=0;
			char line[24];
			//read each line representing HFpoint to std::vector 
		...
		*/
		...
		void load_workload_directed_slice(char* query_pair_file, std::vector<valueType>& queryTime_s,std::vector<valueType>& queryTime_t){
			//read from file
			std::ifstream in(query_pair_file);//input HFPoint file to std::ifstream
			if(!in.is_open()) {std::cerr<<"Cannot open "<<query_pair_file<<std::endl;}
			int s,t;
			valueType qt;
			char line[24];
			while (in.getline(line,sizeof(line))){
		...
		template<typename valueType>
		void load_workload_directed(char* query_pair_path, std::vector<valueType>& queryTime_s,std::vector<valueType>& queryTime_t,int n){
			//initilaize
			if(queryTime_s.empty()) queryTime_s.resize(n,0);
			if(queryTime_t.empty()) queryTime_t.resize(n,0);
			if(is_directory(query_pair_path)){
				std::string query_pairPath(query_pair_path);
				std::vector<std::string> query_pair_files_tmp;
				get_filelist_from_dir(query_pairPath,query_pair_files_tmp);
				append_to_full_path(query_pairPath,query_pair_files_tmp);
				std::cout<<"query_pair_files_tmp.size() = "<<query_pair_files_tmp.size()<<std::endl;
				for(std::string file:query_pair_files_tmp){
					load_workload_directed_slice((char*)file.c_str(),queryTime_s,queryTime_t);
				}
			}else{
				load_workload_directed_slice(query_pair_path,queryTime_s,queryTime_t);
			}
