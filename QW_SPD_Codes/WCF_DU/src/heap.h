/*
 * @Descripttion: define the heap class
 * @version: 1.0
 * @Author: wanjingyi
 * @Date: 2021-02-09 15:29:13
 * @LastEditors: Wan Jingyi
 * @LastEditTime: 2021-10-16 11:08:15
 */
...
template<int log_k, typename k_t,typename k_t_1,typename id_t>
class my_2heap {//heap-based priority-queue which has two key values
public:

	// Expose types.
	typedef k_t key_t;
	typedef k_t_1 key_t_1;
	typedef id_t node_t;
	...
	struct element_t {
		...
		node_t element;
		element_t() : key(0), element(0) {}
		element_t(const key_t k,key_t_1 k1, const node_t e) : key(k),key1(k1), element(e) {}
	};

public:
	// Constructor of the heap.
	my_2heap(node_t n) : n(0), max_n(n), elements(n), position(n, NULLINDEX) {
	}
	...
	// Extract min element.
	inline void extract_min(node_t &element, key_t &key,key_t_1& key1) {
		assert(!empty());
	...
	inline pair<key_t,key_t_1> top() {
		assert(!empty());
		element_t &front = elements[0];
		return make_pair(front.key,front.key1);
	}
	...
	// Update an element of the heap.
	inline void update(const node_t element, const key_t key,const key_t_1 key1) {
		if (position[element] == NULLINDEX) {
			element_t &back = elements[n];
			back.key = key;
			back.key1=key1;
			...
			node_t el_pos = position[element];
			element_t &el = elements[el_pos];
			element_t new_el(key,key1,element);
			if(compare(el,new_el)){
				el.key = key;
				sift_down(el_pos);			
			}else{
				el.key = key;
...
protected:
	//compare function
	bool compare(element_t e1,element_t e2){
		if(e1.key!=e2.key){
	...
	inline void sift_up(node_t i) {
		...
		while (cur_i > 0) {
			node_t parent_i = (cur_i-1) >> log_k;
			if(compare(elements[cur_i],elements[parent_i])) swap(cur_i, parent_i);
			else break;
			cur_i = parent_i;
		}
	}
	...
	inline void sift_down(node_t i) {
		...
		while (true) {
			node_t min_ind = i;
			element_t min_element_t=elements[i];

			node_t child_ind_l = (i << log_k) + 1;
			node_t child_ind_u = std::min(child_ind_l + k, n);

			for (node_t j = child_ind_l; j < child_ind_u; ++j) {
				if(compare(elements[j],min_element_t)){
					min_ind = j;
					min_element_t=elements[j];
				}
			}
