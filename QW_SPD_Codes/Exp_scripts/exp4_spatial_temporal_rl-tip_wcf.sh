...
do
	...
	do
		...
		echo "dataset time_interval_cnt total_labeling_time total_ordering_time total_compute_rmq_time total_label_size" >> ${label_result_file}
		...
		do
			...
			do
				#执行批量构建索引(-q ${query_pair_dir} )
				${EXE_INDEX} -t -d ${DIRECTED_FLAG[d]} -w 1 -s 1 -o 0 -u 0 -m ${INDEXING_PARAS[i]} -n ${DAY_CNT_NUM} -c ${THREADS_NUM_PARAS} -g ${graph_file} -h ${query_freq_dir} -l ${label_dir} -b ${label_size_dir} -i ${inter_file_dir} -k ${tip_file} -p ${paras_file} -a ${label_result_file}
				let CNT++
			done
			#输出size
			...
			do
				#测试查询时间
				${EXE_QUERY} -l -d ${DIRECTED_FLAG[d]} -w 1 -p 1 -b 1 -o 1 -s 3 -x ${SHUFFLE} -n ${NUMQUERY} -m ${INDEXING_PARAS[i]} -i ${label_dir} -e ${label_size_dir} -v ${inter_file_dir} -y ${inter_file_dir} -a ${query_result_file} -q ${query_pair_dir}
				let CNT++
			done
