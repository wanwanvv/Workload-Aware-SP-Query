...
#划分参数
DAY_CNT_NUM=96
TIP_INDEX_PREFIX="GTP"
DATA_NUM=(414 50 50 50) #(414 396 297 180)

#构建索引参数
INDEXING_PARAS=(1) #(1 2)
DIRECTED_FLAG=(0) #(0 1)
DIRECTED_PREFIX=("Undirected") #("Undirected" "Directed")
UPDATE_THRESHOLD=(2.3 1.5 4.0 4.0)
...
do
	...
	do
		...
		do
			...
			do
				#执行批量构建索引 ( -p ${paras_file})
				${EXE_INDEX} -t -d ${DIRECTED_FLAG[d]} -w 1 -s 0 -o 0 -u 0 -m ${INDEXING_PARAS[i]} -c ${THREADS_NUM_PARAS} -g ${graph_file} -h ${query_freq_dir} -q ${query_pair_dir} -l ${label_dir} -b ${label_size_dir} -i ${inter_file_dir} -e ${UPDATE_THRESHOLD[j]} -a ${result_file}
				let CNT++
			done
