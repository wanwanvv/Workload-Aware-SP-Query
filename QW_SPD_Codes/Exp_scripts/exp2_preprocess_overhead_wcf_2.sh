...
do
	...
	result_file=${result_output_prefix}/index.txt
	...
	do
		...
		do
			#执行混合方案构建程序验证最大度对实验结果的影响
			${EXE_INDEX} -z -d ${DIRECTED_FLAG[d]} -w 1 -s 2 -r 0 -o 0 -i ${INDEXING_PARAS[i]} -m ${MAX_DEGREE[j]} -k ${FREQ_PARAS[c_index]} -l ${bet_para} -j 100 -c ${THREADS_NUM_PARAS} -g ${graph_file} -q ${query_freq_file} -t ${inter_file_output_prefix} -a ${result_file} -e ${label_file} -b ${label_size_file}
			let CNT++
		done
		#输出size
