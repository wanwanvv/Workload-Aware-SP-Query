 ...
 * @LastEditTime: 2021-11-09 09:14:17
 */
...
class HFLabel : public Label{
	public:
		...
		void load_hfpoint_and_qt(char* load_filename,int hf_rate=0){//5%%
			...
			numOfHFpoint=0;
            if(is_directory(load_filename)){
                string pointFreqPath(load_filename);
                ...
                for(string file: freqid_files_tmp){
                    load_hfpoint_and_qt_slice((char*)file.c_str(),hf_rate);
                }
            }else{
                load_hfpoint_and_qt_slice(load_filename,hf_rate);
            }
		...
		void load_hfpoint_and_qt_slice(char* load_filename,int hf_rate=0){//5%%
			int numOfHFpoint_slice=0;
			if(hf_rate==0) numOfHFpoint_slice=numOfVertices;
			else numOfHFpoint_slice=static_cast<int>((double)(numOfVertices*hf_rate)/(double)HF_DIVIDION);
			std::ifstream in(load_filename);//input HFPoint file to ifstream
			if(!in.is_open()) {std::cerr<<"Cannot open "<<load_filename<<std::endl;}
			int id,qt,i=0;
			int hf_cnt=0;
		...
		*/
		...
		/*
		 *@description: get the query cost fo batch time slicings from one directory which need to load label size
		 *@author: wanjingyi
		 *@date: 2020-12-29
		*/
		void get_batch_time_slicing_query_cost(char* freqidDirname,char* write_filename)
		{
			HFPoint_rank.resize(numOfVertices);
			HFPoint_inv.resize(numOfVertices);
			std::vector<std::string> _files;
			string queryFreqPath(freqidDirname);
			get_filelist_from_dir(queryFreqPath,_files,true);
			string write_filename_prefix(write_filename);
			string acost_filename=write_filename_prefix.append("_query_cost.values");
			ofstream ofs(acost_filename.c_str());
			if(!ofs.is_open()) {cerr<<"Cannot open "<<acost_filename<<endl;}
			queryTime.resize(numOfVertices,0);
			for(int i=0;i<_files.size();++i){
				string freqidFilename=_files[i];//pure filename
				string freqidFilename_full_prefix(freqidDirname);
				string freqidFilename_full_path=freqidFilename_full_prefix.append(freqidFilename);//full path name
				string ts_id=freqidFilename.substr(freqidFilename.find("_")+1,freqidFilename.find(".")-freqidFilename.find("_")-1);//get the time slicing id
				char* freqid_filename=(char*)freqidFilename_full_path.c_str();
				std::cout<<"freqidFilename ="<<freqidFilename<<" freqid_filename = "<<freqid_filename<<" "<<"ts_id = "<<ts_id<<endl;//to be deleted
				//*******load query time*******
				...
				while (in.getline(line,sizeof(line)))
				{
					...
					if(cnt==0) max_query_time=query_time;
					cnt++;
				}
				std::cout<<"nonempty query time point num = "<<cnt<<endl;
				in.close();
				//*******load query time*******
				double performance_result=0;//total performance function 
				for (NodeID v = 0; v < numOfVertices; ++v) 
				{
					NodeID isize = spt_v_num[v];
					//compute the query performance function
					double ratio=(double)queryTime[v]/(double)DIVISION_FACTOR;
					performance_result+=ratio*(double)isize;
					//std::cout<<"standard performance_result = "<<performance_result<<endl;
				}
				//write performance function to file
				ofs<<ts_id<<" "<<performance_result<<endl;
				//queryTime.clear();//clear tmp
				for(NodeID i=0;i<numOfVertices;++i) queryTime[i]=0;
			}
			ofs.close();
	...
	void load_labels(const char* load_filename) {
		...
		for (NodeID v = 0; v < numOfVertices; ++v) {
			...
			for (NodeID i = 0; i < index_[v].size(); ++i) {
				...
				NodeID parent;
				EdgeWeight hub_weight;
				ifs.read((char*)&hub, sizeof(hub));rf(hub_weight));
				index_[v].spt_v[i] = hub;
...
class HFDLabel : public DLabel {
	...
	//*******************util functions*****************
		...
		*/
		...
		void get_label_size(vector<double>& node_size){
			for (NodeID v = 0; v < numOfVertices; ++v) {
				node_size[v]=bindex_[v].size()-1+bindex_[v].size()-1;
			}
			return;
		}
	 ...
	 */ 
 	void load_hfpoint_and_qt(char* load_filename,int hf_rate=0){
		...
		numOfHFpoint=0;
		if(is_directory(load_filename)){
			string queryPairPath(load_filename);
			vector<string> querypair_files_tmp;
			get_filelist_from_dir(queryPairPath,querypair_files_tmp,true);
			append_to_full_path(queryPairPath,querypair_files_tmp);
			std::cout<<"querypair_files_tmp.size() = "<<querypair_files_tmp.size()<<std::endl;
			for(string file: querypair_files_tmp){
				load_hfpoint_and_qt_slice((char*)file.c_str(),hf_rate);
			}
		}else{
			load_hfpoint_and_qt_slice(load_filename,hf_rate);
		...
		for(size_t v=0;v<numOfVertices;++v){
			if(HFinGraphIndex[v]) ++numOfHFpoint;
	...
	void load_hfpoint_and_qt_slice(char* load_filename,int hf_rate=0){
		std::ifstream in(load_filename);//input HFPoint file to ifstream
		if(!in.is_open()) {std::cerr<<"Cannot open "<<load_filename<<std::endl;}
		int s,t,qt,i=0;
		char line[24];
