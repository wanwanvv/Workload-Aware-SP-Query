/*
 * @Descripttion: pll algorithm
 * @version: 1.0
 * @Author: wanjingyi
 * @Date: 2021-02-09 15:10:20
 * @LastEditors: Wan Jingyi
 * @LastEditTime: 2021-09-21 16:37:10
 */
...
public:
	...
	PL_W(WGraph &wgraph, Ordering &orders) {
		...
		cout << "toal heap size = " << hsize<<endl;
	...
	PL_W(bool isPruned,WGraph &wgraph, Ordering &orders) {
		iteration_generated.resize(numOfVertices);
		pruning_power.resize(numOfVertices);
		vector<index_t>& index_ = labels.index_;
		vector<NodeID> &inv = orders.inv;
	...
	int relabelByOrder(CHGraph &chgraph, Ordering &orders) {
		...
		for (NodeID v = 0; v < numOfVertices; ++v) {
			for (NodeID i = 0; i < chgraph.adj[v].size(); ++i)
				...
				sort(new_adj[rank[v]].begin(),new_adj[rank[v]].end());//written by wanjingyi
			...
			if (DIRECTED_FLAG == true) {
				...
				for (NodeID i = 0; i < chgraph.r_adj[v].size(); ++i)
					...
					sort(new_r_adj[rank[v]].begin(),new_r_adj[rank[v]].end());//written by wanjingyi
			}
		}
...
//				tmp_idx[v].second.clear(); 
	...
	Bottomup(CHGraph &chgraph, Ordering &orders, double& time_contracting) {
		...
		cout<<"relabelByOrder finished......"<<endl; //test written by wanjingyi
		long total_shortcut = 0;
		//time_contracting = GetCurrentTimeSec();
		double time_searching = GetCurrentTimeSec();
		...
		for (int t = 0; t < num_threads; ++t) {
			for (int v = 0; v < numOfVertices; ++v) {
				shortCuts_cnt+= shortcuts_omp[t][v].size(); //test written by wanjingyi
		...
		time_searching = GetCurrentTimeSec() - time_searching;
		cout << "Searching time:" <<time_searching *1e6 <<  " microseconds" << endl;

		//**************test output written by wanjingyi*******
		cout<<"*****************added shortcuts**************"<<endl;
		cout<<"the num of added shortCuts:"<<shortCuts_cnt<<endl;
		for(int r=0;r<numOfVertices;++r){
			cout<<r<<" : ";
			for(int i=0;i<shortcuts[r].size();++i){
				cout<<"("<<shortcuts[r][i].target<<","<<shortcuts[r][i].level<<","<<shortcuts[r][i].weight<<") ";
			}
			cout<<endl;
		}
		cout<<"*****************added shortcuts**************"<<endl;
		...
		time_shortcuting = GetCurrentTimeSec() - time_shortcuting+time_searching;
		cout << "Shortcuting time:" <<time_shortcuting *1e6 <<  " microseconds" << endl;
		...
		if (DIRECTED_FLAG == false)
			...
			labeling_directed(chgraph, orders);
		time_contracting=GetCurrentTimeSec()-time_contracting;
		cout << "Labeling time:" <<time_contracting *1e6 <<  " microseconds" << endl;
		time_contracting=time_contracting+time_shortcuting;
	...
	long process_shortcuts(CHGraph& chgraph, vector<vector<CHGraph::CH_Edge> > shortcuts) {
		...
		for (NodeID v = 0; v < numOfVertices; ++v) {
			...
			for (int i = 0; i < shortcuts_v.size(); ++i) {
				...
				//for(NodeID i=0;i<chgraph.adj[v].size();++i) cout<<chgraph.adj[v][i].target<<" ";
				//cout<<endl;
				//cout<<"pos:"<<"("<<(*pos).target<<","<<(*pos).level<<","<<(*pos).weight<<")"<<endl;
		...
		for (NodeID v = 0; v < numOfVertices; ++v) {
			...
			for (int i = 0; i < shortcuts_v.size(); ++i) {
				...
				if (add_out_v[i] == true)
					...
					cout<<v<<"-("<<w<<","<<level<<","<<weight<<")";//test written by wanjingyi
				if (add_in_v[i] == true)
					chgraph.adj[w].push_back(CHGraph::CH_Edge(v, level, weight));
					cout<<w<<"-("<<v<<","<<level<<weight<<")";//test written by wanjingyi
