 ...
 * @LastEditors: sueRimn
 * @LastEditTime: 2022-02-13 10:36:00
 */
...
public:
	...
	void save_rank(const char* order_dirname) {
		string order_file(order_dirname);
...
class Random_Ordering : public Ordering {
public:

	Random_Ordering(Graph& graph) {
		inv.resize(numOfVertices);
		rank.resize(numOfVertices);

		vector<pair<int, NodeID> > weight(numOfVertices);
		srand(100);
		for (size_t v = 0; v < numOfVertices; ++v) {
			weight[v] = make_pair(rand(), v);//rand() 0~32767
		}
		sort(weight.rbegin(), weight.rend());
		for (size_t v = 0; v < numOfVertices; ++v){
			inv[v] = weight[v].second;
			rank[weight[v].second]=v;
		}
	
		Relabel(graph);	
	}	

	Random_Ordering(WGraph& wgraph) {
		inv.resize(numOfVertices);
		rank.resize(numOfVertices);
...
class Frequency_Ordering : public Ordering{
	...
	//constructor
	Frequency_Ordering(char* load_filename,Graph& graph){
		inv.resize(numOfVertices);
		rank.resize(numOfVertices);
		ifstream in(load_filename);//input HFPoint file to ifstream
		if(!in.is_open()) {cerr<<"Cannot open "<<load_filename<<endl;}
		vector<pair<float, NodeID> > weight(numOfVertices);
		vector<int> freqs(numOfVertices,0);
		srand(100);
		NodeID v;int freq=0;
		//read each line v freq
		while(in>>v>>freq){
			freqs[v]=freq;
		}
		for (size_t v = 0; v < numOfVertices; ++v){
			weight[v]=make_pair(freqs[v]+(float(rand()) / RAND_MAX),v);
		}
		sort(weight.rbegin(), weight.rend());
		for (size_t v = 0; v < numOfVertices; ++v){
			inv[v] = weight[v].second;
	...
	Frequency_Ordering(char* load_filename,WGraph& wgraph){
		inv.resize(numOfVertices);
		rank.resize(numOfVertices);
		ifstream in(load_filename);//input HFPoint file to ifstream
		if(!in.is_open()) {cerr<<"Cannot open "<<load_filename<<endl;}
...
//				for (unordered_set<int>::iterator it = reverse_index[pv].begin(); it != reverse_index[pv].end(); ++it) { // GOOGLE
	...
	Betweenness_Ordering(NodeID k, double beta, WGraph& wgraph, NodeID stopv,double& _ordering_time, double& _hub_time, char* outputBetFileName="",long long bound_times = 10, long long trans_times = 8) {
		double start_time = GetCurrentTimeSec();
		...
		//output betweenness
		if(*outputBetFileName!='\0'){
			string outputBetFile(outputBetFileName);
			outputBetFile.append(".betweenness");
			bet_ofs.open(outputBetFile);
			if(!bet_ofs.is_open()) std::cout<<"Betweenness.values file can't be opened!"<<endl;	
			else isOutputBet=true;
		}
...
public:
	...
	Given_Ordering(char* order_file) {
		...
		cout<<"read given order finished!"<<endl;
	}
