 ...
 * @LastEditTime: 2022-02-19 20:16:19
 */
...
typedef  long long EdgeWeight_l;
typedef  int EdgeWeight;
typedef pair<NodeID, EdgeWeight> NodeEdgeWeightPair;
typedef pair<NodeID, EdgeWeight_l> NodeEdgeWeightPair_l;
...
public:
	...
	bool load_graph(const char* graph_file,int graph_type=0) {
		...
		adj_weight.shrink_to_fit();
		r_adj_weight.shrink_to_fit();
		cout<<"load graph and clear successfully!"<<endl;
		return true;
	}

	/**
	 * @description: load directed weighted graph u v w 
	 * @param {const} char
	 * @param {bool} directed
	 * @return {*}
	 * @author: Wan Jingyi
	*/ 
	/**
	bool load_graph(const char* graph_file, bool is_single_directed){
		cout<<"Start load_graph using single direction way"<<endl;
		ifstream ifs(graph_file);
		vector<Edge> es;
		vector<EdgeWeight> es_weight;
		...
		vertices[numOfVertices] = edges.size();
		cout << sum_of_edges << " edges in forward adj." << endl;
		cout << edges.size() << " edges in forward inserted." << endl;
		cout << duplicated << " edges in forward deleted." << endl;
		...
		if (DIRECTED_FLAG == true) {
			...
			cout << sum_of_edges << " edges in backward adj." << endl;
			cout << r_edges.size() << " edges in backward inserted." << endl;
			cout << duplicated << " edges in backward deleted." << endl;
		}

		//test the rightness of read graph
		//if(numOfEdges_forward+numOfEdges_forward!=numOfEdges) std::cout<<"Error: numOfEdges_forward+numOfEdges_forward!=numOfEdges!!!"<<std::endl;
