 ...
 * @version: 
 * @Author: Wan Jingyi
 * @Date: 2021-11-03 15:49:56
 * @LastEditors: Wan Jingyi
 * @LastEditTime: 2021-11-10 15:04:33
 */
#pragma once
...
class TimeInterval{
    public:
        ...
        TimeInterval(char* graphFileName,char* pointFreqDirName,char* updateIndexFileName,
                                    ...
                                    char* labelSizeDirName,char* betwennessFileName,char* coverageFileName,
                                    char* orderWeightFileName,int numOfDaySlices,int graphType,
                                    char* queryDataDirName){
            load_tip_information(numOfDaySlices,pointFreqDirName,updateIndexFileName);
            make_and_copy_partition_tmp_dirs();
            construct_labels_rl_tip(graphFileName,parasFileName,analyDirName,labelsDirName,labelSizeDirName,betwennessFileName, coverageFileName,orderWeightFileName,graphType);
        ...
        TimeInterval(char* graphFileName,char* pointFreqDirName,char* parasFileName,
                                    ...
                                    char* betwennessFileName,char* coverageFileName,char* orderWeightFileName,
                                    int graphType,double up_threshold,char* queryDataDirName)
        {
            load_gtp_information(pointFreqDirName,up_threshold);
            construct_labels_gtp(graphFileName,parasFileName,analyDirName,labelsDirName,labelSizeDirName,orderWeightFileName,betwennessFileName,coverageFileName,graphType,up_threshold);
            if(*queryDataDirName!='\0') partition_query_wokloads(queryDataDirName);
        }
        ...
        void load_parameters(char* parasFileName,int num){
            int k_freq=0,k_bet=0,k_deg=0,k_cov=0,k_dep=0;
        ...
        void build_whp_index(WGraph& wgraph,const Processing::calcCoefficient<double>& calcCoef,char* orderWeightFileName,char* queryFreqFileName,char* betwennessFileName,char* coverageFileName,char* labelFileName,char* labelSizeFileName,bool is_load_size=false,int hfRate=0){
            double _labeling_time, _hub_time,_ordering_time;//output time
            if(DIRECTED_FLAG==true){
                if(WEIGHTED_FLAG==true){
                    _ordering_time=time_util::GetCurrentTimeSec();
                    Linear_Ordering<double> linear_ordering(wgraph,calcCoef,orderWeightFileName,queryFreqFileName,betwennessFileName,coverageFileName,hfRate);
                    _ordering_time=time_util::GetCurrentTimeSec()-_ordering_time;
                    _hub_time = time_util::GetCurrentTimeSec();
                    PL_W pl_w(wgraph, linear_ordering, DIRECTED_FLAG);
                    _hub_time = time_util::GetCurrentTimeSec()-_hub_time;
                    ...
                    //save label size
                    pl_w.dlabels.save_label_size(labelSizeFileName);
                    if(is_load_size){
                        vector<double> ().swap(_node_size);
                        _node_size.clear();
                        _node_size.resize(numOfVertices,0);
                        pl_w.dlabels.get_label_size(_node_size);
                    }
                }else{
                ...
                if(WEIGHTED_FLAG==true){
                    ...
                    if(is_load_size){
                        ...
                        _node_size.clear();
                        _node_size.resize(numOfVertices,0);
                        pl_w.labels.get_label_size(_node_size);
                    }
                }else{
            ...
            _total_hub_time+=_ordering_time;
            _total_labeling_time+=_ordering_time;
            return;
        }
        
        void construct_labels_rl_tip(char* graphFileName,char* parasFileName,char* analyDirName,
                                                                    char* labelsDirName,char* labelSizeDirName,char* betwennessFileName,
                                                                    char* coverageFileName,char* orderWeightFileName,int graphType){
            //prepare parameters for whp construction of each time interval
            ...
            for(int i=0;i<_time_interval_cnt;++i){
                ...
                build_whp_index(wgraph,_paras[i],orderWeightFileName,(char*) point_freq_dir.c_str(),betwennessFileName,coverageFileName, (char*)label_file_name.c_str(),(char*) label_size_name.c_str());
            }
			...
			ofs.setf(ios::fixed);
			ofs.precision(4);
            ofs<<_time_interval_cnt<<" "<<_total_ordering_time* 1e6 <<" "<<_total_hub_time* 1e6<<" "<<_total_labeling_time* 1e6<<" ";
            ofs.close();
        ...
        void load_gtp_information(char* pointFreqDirName,double up_threshold){
            ...
            cout<<"Load_gip_information successfully!"<<endl;
            return;
        }


        void construct_labels_gtp(char* graphFileName,char* parasFileName,char* analyDirName,
                                                                char* labelsDirName,char* labelSizeDirName,char* orderWeightFileName,
                                                                char* betwennessFileName,char* coverageFileName,int graphType,
                                                                double up_threshold)
        {
            ...
            while(curr_index<_time_slice_cnt){
                ...
                build_whp_index(wgraph,_paras[curr_index],orderWeightFileName,(char*)point_freq_file.c_str(),betwennessFileName,coverageFileName,(char*) label_file_name.c_str(),(char*) label_size_name.c_str(),true);
                //compute query cost
